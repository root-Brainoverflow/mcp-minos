...
들어갔을 때 일어난다. 받아 푼 소스 트리는 동적 단계로만 넘어가는 게 아니라,
정적 소스 스캐너(매니페스트 보안·Semgrep·설명·스키마)의 입력으로도 곧장 쓰여
동적 샌드박스가 뜨기 *전에* 검사된다(`static/findings_runner.py`). 흐름:
...
(스냅샷의 `source_tree_path`가 가리키는 받아 푼 트리는 동적 전달과 별개로, 동적
단계가 뜨기 전에 정적 소스 스캐너—매니페스트 보안·Semgrep·설명·스키마—에도
그대로 입력된다. 소스↔런타임 메타데이터 드리프트(rug-pull) 검사만 런타임
`tools/list`가 필요해 동적 단계 *뒤로* 미뤄진다.)
